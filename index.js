...
io.on('connection',(socket)=>{
  socket.on('createRoom',({name},cb)=>{const code=genCode();const st=baseState(code);st.p1={id:socket.id,name:name||'Player 1'};rooms.set(code,st);socket.join(code);cb&&cb({ok:true,code,state:st});io.to(code).emit('state',st);});
  socket.on('joinRoom',({code,name},cb)=>{code=(code||'').toUpperCase().trim();const st=rooms.get(code);if(!st){cb&&cb({ok:false,error:'Room not found'});return;}if(st.p2){cb&&cb({ok:false,error:'Room full'});return;}st.p2={id:socket.id,name:name||'Player 2'};socket.join(code);cb&&cb({ok:true,code,state:st});io.to(code).emit('state',st);});
  socket.on('changeMatch',({code,matchId})=>{const st=rooms.get(code);if(!st||st.started) return;if(!MATCHES.find(m=>m.id===matchId)) return;st.matchId=matchId;io.to(code).emit('state',st);});
  socket.on('startGame',({code})=>{const st=rooms.get(code);if(!st||!(st.p1&&st.p2)) return;st.started=true;st.p1Strikes=0;st.p2Strikes=0;st.p1Correct=[];st.p2Correct=[];st.turn=1;delete st.lastError;io.to(code).emit('state',st);});
  socket.on('submitGuess',({code,guess})=>{const st=rooms.get(code);if(!st||!st.started) return;const isP1=st.p1&&st.p1.id===socket.id;const isP2=st.p2&&st.p2.id===socket.id;if(!isP1&&!isP2) return;const expectedTurn=st.turn===1?isP1:isP2;if(!expectedTurn) return;const raw=(guess||'').trim();if(!raw) return;const norm0=stripDiacritics(raw);const norm=ALIASES[norm0]||norm0;const ans=ANSWERS[st.matchId];const said=alreadySaidSet(st);const isNew=!said.has(norm);const isCorrect=ans.set.has(norm)&&isNew;if(isCorrect){const disp=ans.show[norm]||raw;if(isP1) st.p1Correct.push(disp); else st.p2Correct.push(disp);st.turn=st.turn===1?2:1;delete st.lastError;} else {const dup=!isNew&&ans.set.has(norm);if(isP1) st.p1Strikes=Math.min(3,st.p1Strikes+1); else st.p2Strikes=Math.min(3,st.p2Strikes+1);st.turn=st.turn===1?2:1;st.lastError=dup?'Already said — strike!':'Not on the starters list.';}io.to(code).emit('state',st);});
  socket.on('resetGame',({code})=>{const st=rooms.get(code);if(!st) return;st.started=false;st.p1Strikes=0;st.p2Strikes=0;st.p1Correct=[];st.p2Correct=[];st.turn=1;delete st.lastError;io.to(code).emit('state',st);});
